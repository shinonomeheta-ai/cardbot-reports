...
# **読み込み元と書き込み先が同じなら、消す前に止める**（2026-09-04 追加）。
#
# 上の控えは「消えた本文を戻す」ための備えで、**なぜ空になったか**は言わない。
# 実際の原因は毎回これだった:
#
#   bash publish-report.sh 2026-09-04-x.md < reports/2026-09-04-x.md
#
# 置き場の本文をそのまま流し込むと、`cat > "$DEST"` が**読む前に中身を捨てる**。
# 名前を突き合わせれば書き込む前に分かるので、ここで名指しで止める。
# 同じ実体を指しているか。`-ef` は device+inode で見るので、`/dev/stdin` や
# 相対パス、シンボリックリンク越しでも当たる（名前の突き合わせでは当たらない）。
same_file() {
  [ -e "$1" ] && [ -e "$2" ] && [ "$1" -ef "$2" ]
}
if [ -n "$SRC" ] && same_file "$SRC" "$DEST"; then
  echo "本文の場所が置き場そのもの: $SRC" >&2
  echo "  そのまま置くと中身が消える。置き場の外に本文を用意して渡すこと。" >&2
  exit 1
fi
if [ -z "$SRC" ] && [ ! -t 0 ] && [ -e /dev/stdin ] && same_file /dev/stdin "$DEST"; then
  echo "標準入力が reports/$NAME そのもの。読む前に中身が消える。" >&2
  echo "  置き場の外へ本文を写してから、第2引数で渡すこと。" >&2
  exit 1
fi
