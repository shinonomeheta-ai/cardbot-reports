...
## 正解表との突き合わせ（本人・2026-09-06 追記）

**正解表（9/2 のまとめ画像・30th CELEBRATION の大手チェーン）の店は、1店残らず
候補台帳にありました。取得・登録・公式根拠は全部できています。**
**落ちているのは P（配信の関門）の `not_reviewed_ec` です。**

本人の突き合わせ結果（原文）:

> | 店 | 候補台帳 | 配布 |
> |---|---|---|
> | 晴れる屋2・古本市場・駿河屋 | ある（公式根拠あり） | not_reviewed_ec で落ちた |
> | ドラゴンスター・ポケモンカードストア | ある | 出ている |
> | 竜のしっぽ・オレタン・シーガル・ファミマ・ビックカメラ・Hobby Zone | ある（公式根拠あり） | 配布に無い |
>
> 落ちているのは P（配信の関門）の not_reviewed_ec です。ci に外すよう指示済み。

この報告の §「候補にはあるが配布に出ていない22チェーン」が、その内訳にあたります。
**「AI 判定を通っていない行」（竜のしっぽ・オレタン等）が関門まで届いているかは
ci が確認します**（このセッションでは触っていません）。

### 位置づけ（本人・原文）

> 取得・登録・根拠が全部動いているのは、3日間の修理の結果です。
> 9/4 の時点では取得が83%捨てられ、鍵が66%引けず、台帳が止まっていた。
> いまは正解表の店が全部台帳にある。最後の1か所が閉じていただけです。
