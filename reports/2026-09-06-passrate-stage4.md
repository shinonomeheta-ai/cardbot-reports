...
## 運用の見張り・追記（2026-09-07 15:10 JST・遅れて発火した 10:10 の schedule）

10:10 の schedule は 15:01 JST に発火した（run [34089049982](https://github.com/shinonomeheta-ai/cardbot/actions/runs/34089049982)・5 時間遅れ）。#1402（復旧の相手は記録にある部品ぜんぶ）の後の最初の check。

- **D1/cardchusen・D1/evidence・D1/meli_melo が復旧として閉じた**（#1402 が効いた）。01:11 の誤報 7 件はこれで全部閉じた。
- ②' は ec-lottery-watch（09:18 の rebase 競合・本物・ci へ）と update-data（昨日 17:58・今日 17:58 に窓を出る）の 2 件が継続。
- ⑦ は L（candidate evidence cache の書き手）と N2（nyuka-watch）に弱い合図。どちらも「その台帳を書くが行を書かない書き手」で設計どおり黙る。毎回出るので、`LANDED` に書き手の一覧を持たせて合図も消すのが次の候補（既出）。
- V5 の店 conflict（#1406）は直近 24 時間の合計では 10% 未満で弱い合図なし（09-07 の日次だけなら 12/118＝10.2%）。
- H2 の弱い合図 2 件（fast 65% → 33%・update-data 19% → 54%）は母集団が小さい（前回の保留 10〜30 回）。従来どおり。
